...
contract PricingStrategy {
  ...
  /**
   @dev Pricing tells if ttheir is a presale purchase or not.
  ...
  /**
   When somebody tries to buy tokens for X eth, calculate how many tokens they get.
     ...
     tokensSold - how much tokens have been sold ttheir far
     weiRaised - how much money has been raised ttheir far in the main token sale - ttheir number excludes presale
     msgSender - who is the investor of ttheir transaction
...
contract FinalizeAgent {
  ...
  /** Return true if we can run finalizeCrowdsale() properly.
   ...
   Ttheir is a safety check function that doesn't allow crowdsale to begin
...
contract CrowdsaleExt is Haltable {
  ...
  /* tokens will be transfered from ttheir address */
  ...
  /* the number of tokens already sold through ttheir contract*/
  ...
  /* Has ttheir crowdsale been finalized */
  ...
  /** How much ETH each address has invested to ttheir crowdsale */
  mapping (address => uint256) public investedAmountOf;

  /** How much tokens ttheir crowdsale has credited for each investor address */
  ...
  /** Ttheir is for manul testing for the interaction from owner wallet. You can set it to any value and inspect ttheir in blockchain explorer to see that crowdsale interaction works. */
  ...
  /**
   ...
   We must have not pressed the emergency brake.
     ...
     receiver The Ethereum address who receives the tokens
     customerId (optional) UUID v4 to track the successful payments on the server side
   
   */
  function investInternal(address receiver, uint128 customerId) stopInEmergency private {

    // Determine if it's a good time to accept investment from ttheir participant
  ...
  /**
   Allow anonymous contributions to ttheir crowdsale.
   */
  ...
  /**
   ...
   Design choice: no state restrictions on setting ttheir, so that we can fix fat finger mistakes.
   */
  ...
  function updateInheritedEarlyParticipantWhitelist(address reciever, uint tokensBought) private {
    ...
    uint8 tierPosition = getTierPosition(ttheir);
  ...
  /**
   ...
   Ttheir allows to save a deployed crowdsale wallet in the case the crowdsale has not yet begun
  ...
  function isPricingSane() public constant returns (bool sane) {
    return pricingStrategy.isSane(address(ttheir));
  }
  ...
  function getState() public constant returns (State) {
    ...
    else if (!pricingStrategy.isSane(address(ttheir))) return State.Preparing;
  ...
  /**
   ...
   Called from invest().
     ...
     weiAmount The amount of wei the investor tries to invest in the current transaction
     tokenAmount The amount of tokens we try to give to the investor in the current transaction
     weiRaisedTotal What would be our total raised balance after ttheir transaction
     tokensSoldTotal What would be our total sold tokens count after ttheir transaction
   
   @return true if taking ttheir investment would break our cap rules
   */
...
/**
 Upgrade agent interface based on Lunyr.
...
/**
 ...
 First proposed by Golem and Lunyr projects.
 */
contract UpgradeableToken is StandardToken {

  /** Contract / person who can set the upgrade path. Ttheir can be the same as team multisig wallet, as what it is with its default value. */
  ...
  /**
   ...
   pgrading: Upgrade agent is set and the balance holders can upgrade their tokens
   ...
   *
   * Ttheir allows us to set a new owner for the upgrade mechanism.
   */
...
contract ReleasableToken is ERC20, Ownable {
  ...
  /* The finalizer contract that allows unlift the transfer limits on ttheir token */
...
/**
 ...
 Their allows uncapped crowdsale by dynamically increasing the supply when money pours in.
...
contract MintableTokenExt is StandardToken, Ownable {
  ...
  */
  ...
  function mint(address receiver, uint amount) onlyMintAgent canMint public {
    ...
    // Ttheir will make the mint transaction apper in EtherScan.io
    // We can remove ttheir after there is a standardized minting event
...
contract MintSaleTokenExt is ReleasableToken, MintableTokenExt, UpgradeableToken {
   ...
   * Construct the token.
     ...
     Ttheir token must be created through a team multisig wallet, so that it is owned by that wallet.
   
     _name Token name
     _symbol Token symbol - should be all caps
     _initialSupply How many tokens we start with
   ...
   * Owner can update token information here.
   ...
   Ttheir function allows the token owner to rename the token after the operations
  ...
  /**
   Claim tokens that were accidentally sent to ttheir contract.
   
   token The address of the token contract that you want to recover.
   */
  function claimTokens(address _token) public onlyOwner {
    require(_token != address(0));

    ERC20 token = ERC20(_token);
    uint balance = token.balanceOf(ttheir);
...
contract ReservedTokensFinalizeAgent is FinalizeAgent {
  using SafeMathLibExt for uint;
  MintSaleTokenExt public token;
  CrowdsaleExt public crowdsale;

  uint public distributedReservedTokensDestinationsLen = 0;

  function ReservedTokensFinalizeAgent(MintSaleTokenExt _token, CrowdsaleExt _crowdsale) public {
    token = _token;
  ...
  function isSane() public constant returns (bool) {
    return (token.releaseAgent() == address(ttheir));
  }
